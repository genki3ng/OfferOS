...
- **备战 · 自带题库**：**5 套角色题库（DS / DE / SWE / PM / ML），合计约 150 道，每题都配可对照的解法**——光 DS 就有 61 道（SQL 20 · Python 17 含 LeetCode/pandas · 统计与实验 · 产品 sense · 行为面 STAR），不是只给个空壳。配 /practice 练习台抽题口述 + 自评（桌面双栏：左题表 / 右题目+要点，点即换；可按**公司/来源**过滤）；文档/速备包里写到的题号（如 `sql-11`）会自动链到练习台对应题，点一下即开。
