...
| 路径 | 内容 |
|---|---|
| `src/lib/data.ts` | 全部解析器：tracker 表 / 公司「当前 opening」/ referrals 表 / 勾选框进度 / journal 条目 / JD 元数据 / HANDOFF 摘要 |
...
- **sprint-plan / 各 prep 文件**：`- [ ]` / `- [x]` 勾选框 → 自动算进度；**站点可直接打勾**（按"全文件第 N 个任务行"定位，文本校验兜底）。sprint-plan 周标题格式 `## Week N（M/D–M/D）：…` → 首页"今日聚焦"卡靠它定位当前周。
- **prep/question-bank.md（练习台题库）**：`## 类别` → `### [id] 题目一行` → 题干补充（可选）→ 单独一行 `**要点**` → 要点内容。加题必须守这个格式。
- **prep/practice-log.md**：4 列表（时间/题/自评/备注），站点自评自动追加行；Claude 据此找薄弱点。
