...
import { getQuestionBank, getPracticeLog } from "@/lib/data";
...
export default function PracticePage() {
  ...
  return (
    <>
      <h1 className="page-title">🏋️ 练习台</h1>
      <p className="page-sub">
        题库 = <Link href="/docs/prep/question-bank">prep/question-bank.md</Link>（{bank.length}{" "}
        题）· 自评自动写 <Link href="/docs/prep/practice-log">practice-log.md</Link>{" "}
        · 我会按记录找薄弱点出补强材料。流程：抽题 → 出声讲 → 看要点 → 自评（→ 可选交批改）。
      </p>
      <PracticeApp questions={questions} stats={stats} />
    </>
  );
}
