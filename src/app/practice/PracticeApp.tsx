...
export default function PracticeApp({
  ...
  const grade = async (g: string) => {
    ...
    setMsg("提交中…");
    try {
      await appendPracticeLog(cur.id, g, "");
    ...
    window.dispatchEvent(
      new CustomEvent("ask-claude", {
        detail: {
          ...
          detail: `题目见 prep/question-bank.md 的 [${cur.id}]。\n\n我的答案（口述转文字）：\n${answer || "（把你的答案粘到这里再提交）"}\n\n请按 mock-interview-bank 自评表 6 维（澄清/结构/技术正确/业务连接/护栏取舍/沟通）打分点评，坑点回填对应 cheatsheet，并把结果记到 practice-log。`,
        },
      })
    );
  };
