...
> 新用户：先看根目录 [GETTING-STARTED.md](GETTING-STARTED.md)（或站内 /start）把它变成你自己的。

## 项目一句话

端到端管理你这次找工作，并让 Claude 在每一步协助。你的「北极星」见 [profile/target.md](profile/target.md)，站点显示的身份/口号在 [`src/site.config.ts`](src/site.config.ts)。
...
6. **收到 prep guide / 面经**（用户上传 PDF、截图或贴文）：出 `prep/briefs/` 速备包，并从中**出练习题进 [prep/question-bank.md](prep/question-bank.md)**（守格式契约，喂 /practice 练习台）。
...
- 网站抓取/工具的可复用 recipe 见 [tools/](tools/)。
