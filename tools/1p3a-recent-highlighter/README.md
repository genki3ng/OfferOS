...
去噪规则全在 `gh.js` 的 `grabFromPage() → cleanForExtraction()`，**不依赖 class 名**，靠的是
一组文本/结构特征（见函数顶部的常量）。页面改版后若某类噪声又漏进来，对照真实 DOM 调相应常量即可：

- 相关帖列表漏进来 → `STAMP` / `CARD_META`（"年份季度 + 就业状态"标签），或第 4 条的 `HOT`（相关帖小标题词）。
- 广告漏进来 → `AD_MARK`（广告专有词）/ `link.1p3a.com` 跳转域。
- 公司关注卡漏进来 → `isWidget`（"主题/帖子 + 回复 + 数字"）。
- 站点导航漏进来 → `NAV_KEYS`（导航专有词）/ 第 5 条链接堆阈值。
- **误删了正文** → 多半是某条规则命中过宽，先看是不是 `cap`（块大小上限）太大、或某关键词太泛；
  实在拿不准就让安全网兜底（去噪结果可疑会自动退回旧的全页抓取，绝不更差）。
...
> **v0.7.0（2026-06-27）**：**大改正文抓取质量**。新版"会员专享版"后台（`/interview/thread/N`）是 SPA，整页**没有干净的 `<article>/<main>` 包住正文** → 旧逻辑退回 `document.body`，把「左栏站点导航 + 中栏相关帖列表(热门面试题 N 题) + 广告 + 公司关注卡 + 回复框」全抓进来，真正的正文淹没在上百行噪声里。现在抓正文时先**克隆页面、按内容安全的规则剥掉这些噪声块**再转 Markdown（相关帖列表 / 站点导航 / 广告与推广卡 / 公司关注卡 / 反应条 / 回复框）。所有判定**不依赖会随改版变化的 class 名**；每条规则只删**最贴合噪声的那一层**、命中不了就保留，并带**安全网**——去噪结果若可疑（太短、丢了标题、超过全页）就**自动退回旧行为**，绝不比从前更差。逻辑在 `gh.js` 的 `grabFromPage() → cleanForExtraction()`，调参见上面「如果正文抓取又变脏了」。**装了旧版的要去 `chrome://extensions` 重新加载扩展才生效。**
>
> **v0.6.2（2026-06-27）**：修**新版"会员专享版"后台（`/interview/thread/N`）截图重名报错**——该界面是 SPA，点列表切帖时 `document.title` **不更新**（停在上一帖），导致每次截图 slug 全相同、同分钟两次抓取撞名报 422。两处修复：① 文件名带上 URL 里的**帖子 ID**（不同帖一定不撞）；② 抓取时优先从内容区取**当前帖标题**而非 stale 的 `document.title`。
>
