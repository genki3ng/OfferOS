...
最后更新：2026-06-27（仍是**模板态**：先把它变成你自己的，再开始真正的求职。下个 session 先读这里。最近一次改动＝立「题库去痕」铁律，见下「已完成」。）
...
- 2026-06-27：**立「题库去痕」铁律——仅针对公开模板**（面试 NDA + 开源）：模板题库非逐字原题、不点名来源公司、零公司标签；`npm run check` 在模板态（`configured:false`）硬拦题目行里的公司名；洗掉 DS 题库里点名真公司的几道题。**用户 fork 配置后（`configured:true`）是私库——题库随意、不受限、不需要去痕。** 练习台「来源/标签」过滤保留给下游自行打标签。详见 journal / CLAUDE.md「开发铁律」。
- 2026-01-15：从模板初始化项目。
