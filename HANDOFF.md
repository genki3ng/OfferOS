...
- 2026-06-27：**立「题库去痕」铁律**（面试 NDA + 开源）——题库非逐字原题、不点名来源公司、**模板零公司标签**；接进 `npm run check` 硬拦（模板态只扫题目行）；洗掉 DS 题库里点名真公司的几道题。决策：**保留**练习台「来源/标签」过滤给下游 fork 自行打标签。详见 journal / CLAUDE.md「开发铁律」。
- 2026-01-15：从模板初始化项目。
